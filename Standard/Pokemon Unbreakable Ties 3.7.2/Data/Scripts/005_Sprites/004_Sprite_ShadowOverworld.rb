...
class Sprite_ShadowOverworld
  ...
  NO_SHADOW_EVENT=["door","nurse","healing balls","Mart","boulder","tree","HeadbuttTree","BerryPlant",".noshadow"]
  ...
  def initialize(sprite,event,viewport=nil)
    ...
    @viewport = viewport
    @disposed = false
    @shadowoverworldbitmap = AnimatedBitmap.new("Graphics/Characters/shadow.png")
  ...
  def update
    ...
    if event.character_name =="" || event.character_name == "nil" ||#(PBTerrain.isGrass?(pbGetTerrainTag(event)) || PBTerrain.hasReflections?(pbGetTerrainTag(event)) || PBTerrain.isSurfable?(pbGetTerrainTag(event)) || PBTerrain.isIce?(pbGetTerrainTag(event))) ||
