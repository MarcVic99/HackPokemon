...
#HiddenMoveHandlers::CanUseMove.add(:FLY,proc { |move,pkmn,showmsg|
  ...
  if $game_player.pbHasDependentEvents?
    pbMessage(_INTL("No puedes usar vuelo mientras hay alguien contigo uwu"))
    return false
  end
  if !GameData::MapMetadata.exists?($game_map.map_id) ||
...
#HiddenMoveHandlers::UseMove.add(:FLY,proc { |move,pokemon|
  ...
  #if !pbHiddenMoveAnimation(pokemon)
    pbSEPlay("silbato_god")
    pbWait(20)
    pbMessage(_INTL("¡Corviknight viene a buscarte!"))
    pbWait(3)
    pbSEPlay("CORVIKNIGHT")
    pbWait(5)
    pbSEPlay("speed fly")
