...
################################################################################################
def pbItemRaiseEV2(pkmn, stat, scene, evGain = 10, item = nil, happiness = "")
  qty = 1
  if Settings::NO_VITAMIN_EV_CAP && pkmn.ev[stat] >= 100
    scene.pbDisplay(_INTL("No tendría ningún efecto."))
    return false
...
def pbResetEVs(pkmn,scene)
  if !scene.pbConfirm(_INTL("Esto reseteará los EVs de {1}. ¿Seguro que quieres usarlo?", pkmn.name))
    return false
  else
    pkmn.ev[:ATTACK] = 0
    pkmn.ev[:DEFENSE] = 0
    ...
    return scene.pbDisplay(_INTL("¡Los stats de {1} se han reseteado!", pkmn.name))
  end
end
...
def pbNatureChangeItem(pkmn,nature,item,scene)
  if [pkmn.nature_for_stats, pkmn.nature].include?(nature)
    ...
    return false if !scene.pbConfirm(_INTL("Esto podría afectar a los stats de {1}. ¿Seguro que quieres usarlo?",pkmn.name))
    ret = pbChangeNature(pkmn,nature)
    if ret
      scene.pbDisplay(_INTL("Los stats de {1} han cambiado debido a la {2}!",pkmn.name,
