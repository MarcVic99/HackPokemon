...
ItemHandlers::UseInField.add(:SACREDASH,proc { |item|
  ...
  pbFadeOutIn {
    ...
    if revived==0
      screen.pbDisplay(_INTL("No tendría ningún efecto."))
    end
    screen.pbEndScene
  }
  next (revived==0) ? 0 : 3
...
ItemHandlers::UseInField.add(:VEGETA,proc { |item|
  if $Trainer.pokemon_count == 0
    pbMessage(_INTL("There is no Pokémon."))
    next 0
  end
...
ItemHandlers::UseInField.add(:HACHANCESTRAL,proc { |item|
  pbMessage(_INTL("¡Has talado {1} árboles!\1", $game_variables[300]))
  next 1
})

ItemHandlers::UseInField.add(:PICO,proc { |item|
  pbMessage(_INTL("¡Has picado {1} minas!\1", $game_variables[301]))
  next 1
})
...
ItemHandlers::UseInField.add(:POKEVIAL,proc { |item|
  pbMessage(_INTL("Has abierto el botiquín."))
  pbCommonEvent(11)
  next 1
})

ItemHandlers::UseInField.add(:DIARIO,proc { |item|
  pbCommonEvent(16)
  next 1
})
...
ItemHandlers::UseOnPokemon.add(:ZUMOCORPHISH,proc { |item,pkmn,scene|
  next pbHPItem(pkmn,100,scene,item)
})

ItemHandlers::UseOnPokemon.add(:CERVEZA,proc { |item,pkmn,scene|
  next pbHPItem(pkmn,120,scene,item)
})

ItemHandlers::UseOnPokemon.add(:ROSQUILLA,proc { |item,pkmn,scene|
  next pbHPItem(pkmn,250,scene,item)
})

ItemHandlers::UseOnPokemon.add(:ESPAGUETIS,proc { |item,pkmn,scene|
  next pbHPItem(pkmn,pkmn.totalhp-pkmn.hp,scene)
})

ItemHandlers::UseOnPokemon.add(:CAKE,proc { |item,pkmn,scene|
  next pbHPItem(pkmn,150,scene,item)
})
...
ItemHandlers::UseOnPokemon.add(:FULLRESTORE,proc { |item,pkmn,scene|
  ...
  pbItemRestoreHP(pkmn,pkmn.totalhp-pkmn.hp)
  pkmn.heal_status
  scene.pbRefresh
...
ItemHandlers::UseOnPokemon.add(:PIZZA,proc { |item,pkmn,scene|
  if pkmn.fainted? || (pkmn.hp==pkmn.totalhp && pkmn.status == :NONE)
    scene.pbDisplay(_INTL("No tendría ningún efecto.."))
    next false
  end
...
ItemHandlers::UseOnPokemon.add(:AZUFRE, proc { |item,pkmn,scene|
  next pbResetEVs(pkmn,scene)
})

ItemHandlers::UseOnPokemon.add(:GOMIMORADO,proc { |item,pkmn,scene|
  next pbItemRaiseEV(pkmn,:HP,scene,10,item,"vitamin")
})

ItemHandlers::UseOnPokemon.add(:ZUMOPS,proc { |item,pkmn,scene|
  next pbItemRaiseEV(pkmn,:HP,scene,20,item,"vitamin")
})
...
ItemHandlers::UseOnPokemon.add(:GOMIROJO,proc { |item,pkmn,scene|
  next pbItemRaiseEV(pkmn,:ATTACK,scene,10,item,"vitamin")
})

ItemHandlers::UseOnPokemon.add(:ZUMOFUERZA,proc { |item,pkmn,scene|
  next pbItemRaiseEV(pkmn,:ATTACK,scene,20,item,"vitamin")
})
...
ItemHandlers::UseOnPokemon.add(:GOMIPLATEADO,proc { |item,pkmn,scene|
  next pbItemRaiseEV(pkmn,:DEFENSE,scene,10,item,"vitamin")
})

ItemHandlers::UseOnPokemon.add(:ZUMODEFENSA,proc { |item,pkmn,scene|
  next pbItemRaiseEV(pkmn,:DEFENSE,scene,20,item,"vitamin")
})
...
ItemHandlers::UseOnPokemon.add(:ZUMOFUERZAESP,proc { |item,pkmn,scene|
  next pbItemRaiseEV(pkmn,:SPECIAL_ATTACK,scene,20,item,"vitamin")
})

ItemHandlers::UseOnPokemon.add(:GOMIAMARILLO,proc { |item,pkmn,scene|
  next pbItemRaiseEV(pkmn,:SPECIAL_ATTACK,scene,10,item,"vitamin")
})
...
ItemHandlers::UseOnPokemon.add(:GOMIVERDE,proc { |item,pkmn,scene|
  next pbItemRaiseEV(pkmn,:SPECIAL_DEFENSE,scene,10,item,"vitamin")
})

ItemHandlers::UseOnPokemon.add(:ZUMODEFENSAESP,proc { |item,pkmn,scene|
  next pbItemRaiseEV(pkmn,:SPECIAL_DEFENSE,scene,20,item,"vitamin")
})
...
ItemHandlers::UseOnPokemon.add(:ZUMOVELOCIDAD,proc { |item,pkmn,scene|
  next pbItemRaiseEV(pkmn,:SPEED,scene,20,item,"vitamin")
})

ItemHandlers::UseOnPokemon.add(:ZUMOLEGENDARIO,proc { |item,pkmn,scene|
 next (pbItemRaiseEV(pkmn,:SPEED,scene,20,item,"vitamin")) && 
      (pbItemRaiseEV2(pkmn,:HP,scene,20,item,"vitamin")) && 
      (pbItemRaiseEV2(pkmn,:ATTACK,scene,20,item,"vitamin")) && 
      (pbItemRaiseEV2(pkmn,:DEFENSE,scene,20,item,"vitamin")) && 
      (pbItemRaiseEV2(pkmn,:SPECIAL_DEFENSE,scene,20,item,"vitamin")) &&
      (pbItemRaiseEV2(pkmn,:SPECIAL_ATTACK,scene,20,item,"vitamin"))
})

ItemHandlers::UseOnPokemon.add(:GOMIAZUL,proc { |item,pkmn,scene|
  next pbItemRaiseEV(pkmn,:SPEED,scene,10,item,"vitamin")
})
...
ItemHandlers::UseOnPokemon.add(:RARECANDY,proc { |item,pkmn,scene|
  ...
  qty = scene.pbChooseNumber(
     _INTL("¿Cuántos {1} quieres usar?", GameData::Item.get(item).name_plural), maximum, 1)
  next false if qty < 1

  # CASOS DE LEVEL PARA NO SOBREPASAR LEVEL
  if (pkmn.level + qty > 14 && !$game_switches[136]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 18 && !$game_switches[4]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 24 && !$game_switches[275]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 26 && !$game_switches[288]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 29 && !$game_switches[317]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 32 && !$game_switches[348]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 36 && !$game_switches[411]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 37 && !$game_switches[450]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end


  if (pkmn.level + qty > 38 && !$game_switches[452]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 42 && !$game_switches[495]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 44 && !$game_switches[508]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 40 && !$game_switches[522]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 45 && !$game_switches[530]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 47 && !$game_switches[546]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 50 && !$game_switches[578]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 43 && !$game_switches[585]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 47 && !$game_switches[615]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 50 && !$game_switches[660]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 51 && !$game_switches[768]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 53 && !$game_switches[806]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 55 && !$game_switches[869]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 70 && !$game_switches[887]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 57 && !$game_switches[889]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end

  if (pkmn.level + qty > 59 && !$game_switches[895]) 
    scene.pbDisplay(_INTL("Ya has alcanzado el límite de nivel hasta el momento."))
    next false
  end
