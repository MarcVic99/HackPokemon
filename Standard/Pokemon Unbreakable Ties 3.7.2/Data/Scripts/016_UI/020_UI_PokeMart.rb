    ...
    loop do
      ...
      if GameData::Item.get(item).is_important?
        ...
        quantity=@scene.pbChooseNumber(
           _INTL("¡Perfecto! ¿Cuantos quieres?",itemname),item,maxafford)
        next if quantity==0
        price*=quantity
        if !pbConfirm(_INTL("Serán ${3}. ¿OK?",
      ...
      if added!=quantity
        ...
        pbDisplayPaused(_INTL("No tienes más sitio en la mochila."))
      else
        ...
        @stock.compact!
        pbDisplayPaused(_INTL("Aqui lo tienes! Gracias!")) { pbSEPlay("Mart buy item") }
   ...
   def pbBuyScreen2
    ...
    loop do
      ...
      if
        ...
        maxafford = Settings::BAG_MAX_PER_SLOT if maxafford > Settings::BAG_MAX_PER_SLOT
        quantity=@scene.pbChooseNumber(
           _INTL("¿Cuantos quieres?",itemname),item,maxafford)
        next if quantity==0
        price*=quantity
      end
      if @adapter.getMoney<price
      ...
      if added!=quantity
        ...
        @stock.compact!
        pbDisplayPaused(_INTL("¡Has comprado {1} {2}!",quantity,itemname)) { pbSEPlay("Mart buy item") }
        if quantity >= 10 && $PokemonBag && GameData::Item.exists?(:PREMIERBALL)
          if Settings::MORE_BONUS_PREMIER_BALLS && GameData::Item.get(item).is_poke_ball?
            premier_balls_added = 0
            (quantity / 10).times do
...
#
#===============================================================================
def pbPokemonMart2(stock,speech=nil,cantsell=false)
  for i in 0...stock.length
  ...
  loop do
      ...
      screen = PokemonMartScreen2.new(scene,stock)
      screen.pbBuyScreen2
break
  end
...
#
#===============================================================================
def pbPokemonMart3(stock,speech=nil,cantsell=false)
  for i in 0...stock.length
    stock[i] = GameData::Item.get(stock[i]).id
    stock[i] = nil if GameData::Item.get(stock[i]).is_important? && $PokemonBag.pbHasItem?(stock[i])
  end
  ...
  commands[cmdQuit = commands.length] = _INTL("Nada, gracias.")
cmd = pbMessage(
     speech ? speech : _INTL("¡Bienvenido! ¿Qué desea tomar?"),
     commands,cmdQuit+1)
  loop do
 if cmdBuy>=0 && cmd==cmdBuy
      scene = PokemonMart_Scene.new
      screen = PokemonMartScreen3.new(scene,stock)
      screen.pbBuyScreen3
 else
      pbMessage(_INTL("¡Vuelve cuando quieras!"))
      break
    end
  cmd = pbMessage(_INTL("¿Necesitas algo más?"),
       commands,cmdQuit+1)
end
  $game_temp.clear_mart_prices
end
...
def pbPokemonMart4(stock,speech=nil,cantsell=false)
  for i in 0...stock.length
    stock[i] = GameData::Item.get(stock[i]).id
    stock[i] = nil if GameData::Item.get(stock[i]).is_important? && $PokemonBag.pbHasItem?(stock[i])
  end
...
cmd = pbMessage(
 ...
 if cmdBuy>=0 && cmd==cmdBuy
      scene = PokemonMart_Scene.new
      screen = PokemonMartScreen4.new(scene,stock)
      screen.pbBuyScreen4
 else
      pbMessage(_INTL("¡Vuelve cuando quieras!"))
      break
