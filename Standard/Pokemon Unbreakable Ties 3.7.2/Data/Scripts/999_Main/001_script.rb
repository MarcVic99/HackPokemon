...
class Hatcher
  ...
  def refresh
    ...
    if eggs[@index] != nil
      ...
      eggstate=_INTL("Parece que va a tardar en abrirse...")
      eggstate=_INTL("¿Qué saldrá de este huevo? Todavía le queda para abrirse...") if steps<10200
      eggstate=_INTL("Parece que a veces se mueve. Quizás le falte poco para abrirse.") if steps<2550
      eggstate=_INTL("¡Se escuchan ruidos desde dentro! ¡Le falta poco para abrirse!") if steps<1275
    ...
    loop do
      ...
      if Input.trigger?(Input::C)
        if $PokemonGlobal.eggs[@index] == nil
          ret = Kernel.pbConfirmMessage("El hueco está disponible.\\n¿Quieres añadir un huevo?")
          if ret == true
            chosen=0
            pbFadeOutIn(99999){
            ...
            if $Trainer.party[chosen] != nil
              if !$Trainer.party[chosen].egg?
                Kernel.pbMessage("El Pokémone escogido, no es un huevo.")
              else
          ...
          ret = Kernel.pbConfirmMessage("¿Quieres retirar este huevo de la incubadora?")
          if ret == true
...
def takeEgg(egg,index)
  sel = Kernel.pbConfirmMessage(_INTL("¿Quieres añadir el huevo en tu equipo?"))
  if sel==true
    Kernel.pbMessage(_INTL("¡Tu equipo está lleno!\1")) if $Trainer.party.length == 6
    pbStorePokemon(egg)
  else
    if pbBoxesFull?
      Kernel.pbMessage(_INTL("¡No hay más hueco en tu caja!\1"))
    ...
    if storedbox!=oldcurbox
      if creator
        Kernel.pbMessage(_INTL("La caja \"{1}\" está llena.\1",curboxname,creator))
      else
...
if defined?(Settings::EGG_LEVEL)
  ...
  def pbGenerateEgg(pkmn, text = "")
    ...
    if (GameData::Item.exists?(:EGGHATCHER) && $PokemonBag.pbHasItem?(:EGGHATCHER)) || $game_switches[EGGHATCHER_SWITCH]
      ret = Kernel.pbConfirmMessage("¿Quieres añadir el Pokémon en la incubadora?")
      if ret == true
        ret = addEgg(pkmn)
        if ret == true
          return true
...
#AQUI DEBERIA METER LO DE CONTROLAR EL PSJ EN X INSTANCIA.
if $game_switches[510] == false && $game_switches[610] == false && $game_switches[613] == false
