...
class RouletteScene
  ...
  def startScene(wager, backgroundColor, usedIconColor)
    ...
    displayMessage(
      _INTL("Selecciona la casilla o casillas en la que creas que va a caer la bola.")
    )
    drawMultiplier
  end
  ...
  def processResults
    ...
    if wins
      ...
      if multiplier==12
        ...
        displayMessage(_INTL(
          "\\me[{1}]¡Le diste!\\wtnp[30]",
          RouletteBridge.getAudioName("Slots win")
        ))
      end
      addCredits(@wager*multiplier, false)
      displayMessage(_INTL("¡Has ganado {1} fichas!",@wager*multiplier))
    else  
      pbPlayBuzzerSE()
      displayMessage(_INTL("¡F en el chat!"))
    end
    @playedBalls[@result]=true
    if RouletteScreen.count(@playedBalls,true)==(ROUNDS-1) # Clear
      displayMessage(_INTL("¡Se va a limpiar la mesa!"))
      @roulette.clearBalls
      @playedBalls.clear
    ...
    drawMultiplier
    if confirmMessage(_INTL("¿Quieres seguir jugando?"))
      if RouletteBridge.coins<@wager
        displayMessage(_INTL("¡No tienes suficientes fichas para jugar!"))
        @exit=true 
      end
    else  
      @exit=true 
...
def roulette(wager=1, backgroundAndIconColor=nil)
  if !RouletteBridge.hasCoinCase?
    RouletteBridge.message(_INTL("Estás en la ruleta, pero no tienes monedero para jugar."))
  elsif RouletteBridge.confirmMessage(_INTL(
    "\\CNEl mínimo de apuesta es de {1} fichas. ¿Quieres jugar?", wager
  ))
    if RouletteBridge.coins<wager
      RouletteBridge.message(_INTL("¡No tienes suficientes fichas para jugar!"))
    elsif RouletteBridge.coins==RouletteBridge.maxCoins
      RouletteBridge.message(_INTL("¡Tu monedero está a full!"))  
    else
      pbFadeOutIn(99999){     
