...
#    if pbConfirmMessageSystemModern(0,false,_INTL("Quieres guardar la partida?"))
      if safeExists?(RTP.getSaveFileName("Game.rxdata"))
        if $PokemonTemp.begunNewGame
          pbMessage(_INTL("¡CUIDADO!"))
          pbMessage(_INTL("Ya hay una partida guardada."))
          pbMessage(_INTL("Si guardas, todo tu progreso se perderá :c"))
          if !pbConfirmMessageSerious(
             _INTL("¿Estás seguro que quieres sobreescribir la partida?"))
      ...
      pbSEPlay("GUI save choice")
      if pbSave
        pbMessage(_INTL("\\se[]{1} ha guardado la partida.\\me[GUI save game]\\wtnp[30]",$Trainer.name))
        ret=true
      else
        pbMessage(_INTL("\\se[]No se ha podido guardar la partida.\\wtnp[30]"))
        ret=false
      end
    else
      pbSEPlay("GUI save choice")
