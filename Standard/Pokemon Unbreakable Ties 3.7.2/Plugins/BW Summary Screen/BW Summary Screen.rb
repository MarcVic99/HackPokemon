...
#=============================================================================
  ...
  def drawPage(page)
    ...
    if @pokemon.hasItem?
      ...
      textpos.push([_INTL("Nada."),290,344,0,base,shadow])
    end
  ...
  def drawPageOneEgg
    ...
    eggstate = _INTL("Parece que a este Huevo le queda mucho tiempo para abrirse.")
    eggstate = _INTL("¿Qué saldrá de aquí? Aún le queda para abrirse.") if @pokemon.steps_to_hatch < 10200
    eggstate = _INTL("A veces se mueve. Debería abrirse pronto.") if @pokemon.steps_to_hatch < 2550
    eggstate = _INTL("Está haciendo ruidos... ¡Parece que está a punto de abrirse!") if @pokemon.steps_to_hatch < 1275
    memo += sprintf("<c3=404040,B0B0B0>%s\n",eggstate)
    # Draw all text
  ...
  def drawPageTwo
    ...
    mettext = [_INTL("Encontrado al Lvl. {1}.",@pokemon.obtain_level),
               _INTL("Huevo recibido."),
               _INTL("Intercambiado al Lvl. {1}.",@pokemon.obtain_level),
               "",
               _INTL("Tuvo un fatídico encuentro al Lvl. {1}.",@pokemon.obtain_level)
    ...
    # If Pokémon was hatched, write when and where it hatched
        ...
        # Changed the color of the text, to the one used in BW
        #memo += _INTL("<c3=404040,B0B0B0>{1} {2}, {3}",date,month,year)
      #end
    ...
    if showNature
      ...
      characteristics = {
        :HP              => [_INTL("Le encanta comer al gordito."),
                             _INTL("Se echa muchas siestas."),
                             _INTL("Suele perder cosas..."),
                             _INTL("A veces se enfada,"),
                             _INTL("Le encanta relajarse.")],
        :ATTACK          => [_INTL("Orgulloso de su fuerza."),
                             _INTL("Le gusta revolverse."),
                             _INTL("Un poco cabezota."),
                             _INTL("Le encanta darse de ostias."),
                             _INTL("Muy muy cabezota.")],
        :DEFENSE         => [_INTL("Cuerpo resistente."),
                             _INTL("Capaz de hacerlo todo solo."),
                             _INTL("Realmente resistente."),
                             _INTL("Buen fajador."),
        ...
        :SPECIAL_DEFENSE => [_INTL("Voluntad fuerte."),
                             ...
                             _INTL("Nada es un juego para él."),
                             _INTL("Algo testarudo.")],
        :SPEED           => [_INTL("Le gusta correr-se."),
                             _INTL("Alerta de sonidos."),
                             _INTL("Impetuoso y pelotudo."),
                             _INTL("Un poco peepoClown."),
                             _INTL("Rápido para huir.")]
      }
      memo += sprintf("<c3=404040,B0B0B0>%s\n", characteristics[best_stat][best_iv % 5])
    end

    memo += _INTL("Amistad -> <c3=0000d6,7394ff>{1}<c3=404040,B0B0B0>.\n",@pokemon.happiness)

    # TODO: PONER EVOLUCIÓN:

    def getEvolutionMessage(evolution, method, parameter)
      evoName = GameData::Species.get(evolution).name
      ret = case method
        when :Level
          _INTL("{1} a Lvl <c3=0000d6,7394ff>{2}", evoName,parameter)
        when :LevelMale
          _INTL("{1} a Lvl <c3=0000d6,7394ff>{2}<c3=404040,B0B0B0> y debe ser macho", evoName,parameter)
        when :LevelFemale
          _INTL("{1} a Lvl <c3=0000d6,7394ff>{2}<c3=404040,B0B0B0> y debe ser hembra", evoName,parameter)
        when :LevelRain
          _INTL("{1} a Lvl <c3=0000d6,7394ff>{2}<c3=404040,B0B0B0> cuendo llueve", evoName,parameter)
        when :DefenseGreater
          _INTL("{1} a Lvl <c3=0000d6,7394ff>{2}<c3=404040,B0B0B0> y ATK > DEF",evoName,parameter)
        when :AtkDefEqual
          _INTL("{1} a Lvl <c3=0000d6,7394ff>{2}<c3=404040,B0B0B0> y ATK = DEF",evoName,parameter) 
        when :AttackGreater
          _INTL("{1} a Lvl <c3=0000d6,7394ff>{2}<c3=404040,B0B0B0> y DEF < ATK",evoName,parameter)
        when :Silcoon,:Cascoon
          _INTL("{1} at level {2} with personalID", evoName,parameter)
        when :Ninjask
          _INTL("{1} a Lvl <c3=0000d6,7394ff> {2}",evoName,parameter)
        when :Shedinja
          _INTL("{1} a Lvl <c3=0000d6,7394ff>{2}<c3=404040,B0B0B0> con un hueco al final",evoName,parameter)
        when :Happiness
          _INTL("{1} cuando es feliz",evoName)
        when :HappinessDay
          _INTL("{1} cuando es feliz durante el día",evoName)
        when :HappinessNight
          _INTL("{1} cuando es feliz durante la noche",evoName)
        when :Beauty
          _INTL("{1} when beauty is greater than {2}",evoName,parameter) 
        when :DayHoldItem
          _INTL("{1} llevando <c3=0000d6,7394ff>{2}<c3=404040,B0B0B0> durante el día",evoName,GameData::Item.get(parameter).name)
        when :NightHoldItem
          _INTL("{1} llevando <c3=0000d6,7394ff>{2}<c3=404040,B0B0B0> durante la noche",evoName,GameData::Item.get(parameter).name)
        when :HasMove
          _INTL("{1} cuando tiene el movimiento <c3=0000d6,7394ff>{2}",evoName,GameData::Move.get(parameter).name)
        when :HappinessMoveType
          _INTL("{1} cuando es feliz con el movimiento <c3=0000d6,7394ff>{2}.",evoName,GameData::Type.get(parameter).name)
        when :HasInParty
          _INTL("{1} when has {2} at party",evoName,GameData::Species.get(parameter).name)
        when :Location
          _INTL("{1} at {2}",evoName, pbGetMapNameFromId(parameter))
        when :Item
          _INTL("{1} -> <c3=0000d6,7394ff>{2}",evoName,GameData::Item.get(parameter).name)
        when :ItemMale
          _INTL("{1} -> <c3=0000d6,7394ff>{2}<c3=404040,B0B0B0> y es macho",evoName,GameData::Item.get(parameter).name)
        when :ItemFemale
          _INTL("{1} -> <c3=0000d6,7394ff>{2}<c3=404040,B0B0B0> y es hembra",evoName,GameData::Item.get(parameter).name)
        when :Trade
          _INTL("{1} tradeando",evoName)
        when :TradeItem
        ...
        when :Gimmighoul
          _INTL("{1} -> <c3=0000d6,7394ff>{2}<c3=404040,B0B0B0> y min lvl <c3=0000d6,7394ff>35" ,evoName,GameData::Item.get(parameter).name)
        when :Riolu
          _INTL("Su poder debe despertar...")
        when :Combusken
          _INTL("Su poder debe despertar...")
        when :Grovyle
          _INTL("Su poder debe despertar...")
    ...
    @data = pbGetEvolvedFormData(@pokemon.species, false)
    ...
    memo += _INTL("<c3=C60000,FF7373>EVO:<c3=404040,B0B0B0> {1}\n",evolutionsStrings.empty? ? _INTL("No evolucionará más.\n") : evolutionsStrings[0])
    evolutionsStrings.shift

    for indice in 0..(evolutionsStrings.length - 1)
      memo += _INTL("<c3=C60000,FF7373>EVO:<c3=404040,B0B0B0> {1}\n",evolutionsStrings.empty? ? _INTL("No evolucionará más.\n") : evolutionsStrings[indice])
    end
  ...
  def drawPageThree
    ...
    def changeColorIVs(value, red, blue, base)
      if value >= 28
        return red
      elsif value <= 5
        return blue
    ...
    def isShadow(value, base, shadow)
      if value >= 28 || value <= 5
        return base
      else
        return shadow
...
# Write various bits of text
    if SHOW_EV_IV
      textpos = [
        ...
        [sprintf("%d",@pokemon.iv[:HP]),296,70,2,changeColorIVs(@pokemon.iv[:HP], red, blue, base),isShadow(@pokemon.iv[:HP], base, shadow)],
        ...
        [sprintf("%d",@pokemon.iv[:ATTACK]),296,120,2,changeColorIVs(@pokemon.iv[:ATTACK], red, blue, base),isShadow(@pokemon.iv[:ATTACK], base, shadow)],
        ...
        [sprintf("%d",@pokemon.iv[:DEFENSE]),296,152,2,changeColorIVs(@pokemon.iv[:DEFENSE], red, blue, base),isShadow(@pokemon.iv[:DEFENSE], base, shadow)],
        ...
        [sprintf("%d",@pokemon.iv[:SPECIAL_ATTACK]),296,184,2,changeColorIVs(@pokemon.iv[:SPECIAL_ATTACK], red, blue, base),isShadow(@pokemon.iv[:SPECIAL_ATTACK], base, shadow)],
        ...
        [sprintf("%d",@pokemon.iv[:SPECIAL_DEFENSE]),296,216,2,changeColorIVs(@pokemon.iv[:SPECIAL_DEFENSE], red, blue, base),isShadow(@pokemon.iv[:SPECIAL_DEFENSE], base, shadow)],
        ...
        [sprintf("%d",@pokemon.iv[:SPEED]),296,248,2,changeColorIVs(@pokemon.iv[:SPEED], red, blue, base),isShadow(@pokemon.iv[:SPEED], base, shadow)],
