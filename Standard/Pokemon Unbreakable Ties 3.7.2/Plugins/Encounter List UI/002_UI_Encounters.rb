...
:OldRod => "Pesca (Caña vieja)",
:GoodRod => "Pesca (Caña buena)",
:SuperRod => "Pesca (Supercaña)",
...
class EncounterList_Scene
  ...
  def drawPresent
    ...
    loctext += sprintf("<al><c2=7FF05EE8>Encuentros de ruta: %s</c2></al>",enc_array.length)
  ...
  def drawAbsent
    ...
    loctext += sprintf("<al><c2=7FF05EE8>¡Este área no tiene Pokémon salvajes!</c2></al>")
    loctext += sprintf("<c2=63184210>-----------------------------------------</c2>")
    @sprites["locwindow"].setText(loctext)
  end
