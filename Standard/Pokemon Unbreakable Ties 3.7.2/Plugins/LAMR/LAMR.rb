...
class MoveRelearnerScreen
  ...
  def pbStartScreen(pkmn)
    ...
    loop do
      ...
      elsif @scene.pbConfirm(_INTL("Dejar de enseñar el movimiento a {1}?", pkmn.name))
        @scene.pbEndScene
        return false
      end
    end
...
class PokemonPartyScreen
  def pbPokemonScreen
    @scene.pbStartScene(@party,
      (@party.length>1) ? _INTL("Escoge un Pokémon.") : _INTL("Escoge un Pokémon o cancela."),nil)
    loop do
      @scene.pbSetHelpText((@party.length>1) ? _INTL("Escoge un Pokémon.") : _INTL("Escoge un Pokémon o cancela."))
      ...
      cmdItem    = -1
      # Build the commands
      commands[cmdSummary = commands.length]      = _INTL("Resumen")
      commands[cmdRelearn = commands.length]      = _INTL("Recuerda Mov.")  #by Kota
      ...
      commands[cmdSwitch = commands.length]       = _INTL("Cambiar") if @party.length>1
      if !pkmn.egg?
        if pkmn.mail
	...
	#LO AÑADIDO
	commands[cmdRename = commands.length]     = _INTL("Mote")
      end
      commands[commands.length]                   = _INTL("Cancelar")
      command = @scene.pbShowCommands(_INTL("¿Qué quieres hacer con {1}?",pkmn.name),commands)
      havecommand = false
      cmdMoves.each_with_index do |cmd, i|
        ...
        if [:MILKDRINK, :SOFTBOILED].include?(pkmn.moves[i].id)
          ...
          @scene.pbSetHelpText(_INTL("En qué Pokémon usar?"))
          oldpkmnid = pkmnid
          loop do
            ...
            if pkmnid==oldpkmnid
              pbDisplay(_INTL("¡{1} no puede usar {2} sobre sí mismo!",pkmn.name,movename))
            elsif newpkmn.egg?
              pbDisplay(_INTL("¡{1} no puede ser usado sobre un huevo!",movename))
            elsif newpkmn.hp==0 || newpkmn.hp==newpkmn.totalhp
              pbDisplay(_INTL("{1} no puede ser usado sobre este Pokémon.",movename))
            else
              pkmn.hp -= amt
          ...
          if pbConfirmUseHiddenMove(pkmn,pkmn.moves[i].id)
            ...
            if pkmn.moves[i].id == :FLY
              ...
              @scene.pbStartScene(@party,
                (@party.length>1) ? _INTL("Escoge un Pokémon.") : _INTL("Escoge Pokémon o Cancela."))
              break
            end
            return [pkmn,pkmn.moves[i].id]
          end
      ...
      if cmdSummary>=0 && command==cmdSummary
        ...
        if MoveRelearnerScreen.pbGetRelearnableMoves(pkmn).empty?
          pbDisplay(_INTL("Este Pokémon no tiene movimientos por recordar."))
        else
        ...
        @scene.pbSetHelpText(_INTL("¿A dónde lo quieres mover?"))
        oldpkmnid = pkmnid
        ...
        when 0   # Read
          pbFadeOutIn {
            ...
            @scene.pbSetHelpText((@party.length>1) ? _INTL("Escoge un Pokémon") : _INTL("Choose Pokémon or cancel."))
          }
        when 1   # Take
        ...
        itemcommands[cmdTakeItem=itemcommands.length] = _INTL("Coger") if pkmn.hasItem?
        itemcommands[cmdMoveItem=itemcommands.length] = _INTL("Mover") if pkmn.hasItem? &&
                                                                        !GameData::Item.get(pkmn.item).is_mail?
        itemcommands[itemcommands.length]             = _INTL("Cancelar")
...
##################################################################################
	...
	if cmdRename>=0 && command==cmdRename
